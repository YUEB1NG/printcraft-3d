...
export default function ReliefStudio() {
  ...
  const [p, setP] = useState({ depth: 0.5, contrast: 1.1, base: 0.05, glow: 1 })
  ...
  useEffect(() => {
    setP((prev) => ({
      ...
      glow: params.get('g') ? +params.get('g')! : prev.glow,
    }))
  }, [])
  ...
  function onFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    const img = new Image()
    img.onload = () => {
      try { setRelief(sampleImage(img, 160)) }
  ...
  function copyLink() {
    const q = new URLSearchParams({ d: String(p.depth), c: String(p.contrast), g: String(p.glow) })
    const link = `${location.origin}${location.pathname}#/relief?${q.toString()}`
  ...
  function submitLead(e: FormEvent) {
    ...
    all.push({ type: 'relief', ...lead, params: p, at: Date.now() })
  ...
  const stage = (
    <Stage>
      {relief ? (
        <ReliefMesh field={relief.field} cols={relief.cols} rows={relief.rows} depth={p.depth} contrast={p.contrast} base={p.base} glow={p.glow} />
      ) : null}
    </Stage>
  )
    ...
    <StudioShell
          ...
          {!relief && (
            <div style={overlayWrap}>
              ...
              <div style={{ fontWeight: 600, marginBottom: 6 }}>上传一张照片开始</div>
                <div style={{ color: 'var(--text-dim)', fontSize: 13 }}>人像、风景、宠物都行 · 越清晰越好</div>
      ...
      panel={
        <div>
          ...
          <Slider label="浮雕深度" min={0.1} max={1.2} value={p.depth} onChange={(v) => setP({ ...p, depth: v })} format={(v) => v.toFixed(2)} />
          <Slider label="对比度" min={0.5} max={2} value={p.contrast} onChange={(v) => setP({ ...p, contrast: v })} format={(v) => v.toFixed(2)} />
          <Slider label="底光强度" min={0} max={2} value={p.glow} onChange={(v) => setP({ ...p, glow: v })} format={(v) => v.toFixed(2)} />
